...
const executeComponentInstall = (componentName, npmInstallURL) => {
  ...
  listComponentsAvalailable().then((componentsList) => {
    ...
    if (componentsList.includes(componentName)) {
      executeNPMInstall(npmInstallURL);
    } else {
...
const executeNPMInstall = (npmInstallURL) => {
  exec(`npm install ${npmInstallURL}`, (error, stdout, stderr) => {
    console.log("Dependencies installed");

    if (error) {
      console.error(
        `Error during component ${componentName} install: ${error}`
      );
      process.exit(1);
    }
    console.log(stdout);
...
const listComponentsAvalailable = async () => {
  const githubContentsURL =
    "https://api.github.com/repos/Eduk29/poc-design-system-angular-core/contents/dist?ref=develop";
